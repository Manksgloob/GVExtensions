		...
		if(bres2){
			...
			if(strLicense && [strLicense length]>0){
				...
				if(l1==limei){
					if(l2==0){
						_licenseStatus = TRUE;
						//						[[[[UIAlertView alloc] initWithTitle:@"Phone GV Extension" message:@"License Status: Registered."
...
//		UIActionSheet *act = [[[UIActionSheet alloc] initWithTitle:nil delegate:callMethodDelegate cancelButtonTitle:@"Cancel" destructiveButtonTitle:@"GV Direct Dial" otherButtonTitles:@"GV Call Back",@"GV Offline Dial",@"Carrier",nil] autorelease];
		...
		if(strLicense && [strLicense length]>0){
			...
			while((c=*buf++)){
				...
				if(c>'5'){
					...
					if(!rand){
						if(num0==0 || (num%num0)){
							// invalid license found
							[[[[UIAlertView alloc] initWithTitle:@"Phone GV Extension" message:@"Obtain License from Settings | Phone GV Extension | Check License."
														delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] autorelease] show];				
			...
			if(l1==limei){
				if(l2==0){
					...
					NSString *devId = __getDeviceID();
					NSString *message = [NSString stringWithFormat:@"License Status: Trial: %d Day(s).\nDevice ID: %@\ngvexts.appspot.com/phone", nDays, devId];
					UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:@"Phone GV Extension" message:message delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] autorelease];
					[alert show];	
				}else if(l3==0 || _now <= l3-86400){
					[[[[UIAlertView alloc] initWithTitle:@"Phone GV Extension" message:@"Obtain License from Settings | Phone GV Extension | Check License."
												delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] autorelease] show];				
...
//					// expired
					_licenseStatus = FALSE;
					[[[[UIAlertView alloc] initWithTitle:@"Phone GV Extension" message:@"License Expired."
																	   delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] autorelease] show];
				}
			}else{
				// invalid license found
...
void PhoneGVExtensionHookInitialize() {
